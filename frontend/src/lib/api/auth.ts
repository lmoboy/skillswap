...
async function handleResponse<T>(response: Response): Promise<T> {
   ...
   if (!response.ok) {
      ...
      throw new Error(
         data.message || 'Pieprasījums neizdevās (angļu v. Request failed)',
      )
   }
   return data
}
...
export async function login(credentials: {
   ...
   try {
      ...
      if (!response.ok) {
         throw new Error(
            data.error || 'Pieteikšanās neizdevās (angļu v. Login failed)',
         )
      }

      auth.setStep(
         'Lietotājs iestatīts, pārbauda autentifikāciju... (angļu v. User set, checking auth...)',
      )
      // Login endpoint only returns status, get user data from checkAuth
      const isAuthenticated = await checkAuth()
      if (!isAuthenticated) {
         throw new Error(
            'Autentifikācija neizdevās pēc pieteikšanās (angļu v. Authentication failed after login)',
         )
      }
      ...
      const errorMessage =
         error instanceof Error
            ? error.message
            : 'Pieteikšanās neizdevās (angļu v. Login failed)'
...
export async function register(credentials: {
   ...
   try {
      ...
      if (data) {
         ...
         throw new Error(
            'Nederīgs atbildes formāts no servera (angļu v. Invalid response format from server)',
         )
      }
      auth.setStep('Pieteikšanās pabeigta (angļu v. Login done)')
...
export async function logout(): Promise<void> {
   try {
      ...
      auth.setStep(
         'Žetona noņemšana no db... (angļu v. Removing token from db...)',
      )
      ...
      if (!response.ok) {
         ...
         throw new Error(
            data.error || 'Izrakstīšanās neizdevās (angļu v. Logout failed)',
         )
      }
   } catch (error) {
      const errorMessage =
         error instanceof Error
            ? error.message
            : 'Izrakstīšanās neizdevās (angļu v. Logout failed)'
...
export async function checkAuth(): Promise<boolean> {
   try {
      ...
      if (!response.ok) {
         auth.setStep(
            'Atbildē radās kļūda... (angļu v. Error occured with response...)',
         )

         // For unauthorized responses, clear user state
         if (response.status === 401) {
            console.warn(
               'Nav autentificēts:',
               data.error || 'Nav kļūdas detaļu',
            )
        ...
        console.warn(
           response.status,
            'Kļūda pārbaudot autentifikāciju:',
            data.error || 'Servera kļūda',
         )
         return auth.isAuthenticated() // Return current auth state
      }

      if (data) {
         auth.setStep(
            'Lietotāja datu iestatīšana... (angļu v. Set user data...)',
         )
      ...
      auth.setStep(
         'Dati neizdevās, notīra... (angļu v. Data failed, clearing...)',
      )
      ...
      const errorMessage =
         error instanceof Error
            ? error.message
            : 'Nezināma kļūda (angļu v. Unknown error)'
      auth.setStep('Autentifikācijas pārbaude neizdevās:' + errorMessage)
      // Don't clear user on network errors - they might still be authenticated
      return auth.isAuthenticated() // Return current auth state
   } finally {
